...
class FunSetSuite extends FunSuite {
  ...
  test("intersect contains elements in every sets") {
    new TestSets {
      val s = union(s1, s2)
  ...
  test("for all true") {
    ...
    new TestSets {
      val s = union(s1, s2)
      val i: Boolean = forall(s, x => contains(s,x))
  ...
  test("for all false") {
    ...
    new TestSets {
      val s = union(s1, s2)
      val i: Boolean = forall(s, x => contains(s1,x))
  ...
  test("exists true") {
    ...
    new TestSets {
      val s = union(s1, s2)
      val i: Boolean = exists(s, x => contains(s,x))

      assert(!i, "exists false")
    }
  }
  test("exists false") {
    ...
    new TestSets {
      val s = union(s1, s2)
      val t = singletonSet(10000)
      val u = union(s,t)
      val i: Boolean = exists(u, x => contains(s1,x))
